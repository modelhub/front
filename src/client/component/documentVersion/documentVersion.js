                    ...
                    controller: ['$element', '$location', '$rootScope', '$scope', '$window', 'api', 'currentUser', 'EVENT', 'i18n', function($element, $location, $rootScope, $scope, $window, api, currentUser, EVENT, i18n){
                        ...
                        loadNextTreeNodeBatch = function(){
                            if(filter && !loadingNextTreeNodeBatch) {
                                ...
                                if (filter !== 'document') {
                                    api.v1.treeNode.getChildren($scope.documentVersionId, filter, offset, limit, 'nameAsc').then(successCallback, errorCallback);
                                } else {
                                    api.v1.helper.getChildrenDocumentsWithLatestVersionAndFirstSheetInfo($scope.documentVersionId, offset, limit, 'nameAsc').then(successCallback, errorCallback);
                                }
                            }
                        };
